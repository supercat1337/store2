...
import { Atom, batch, Collection, Computed, untrack } from "../index.js";
...
test("Collection: with compareFunction", (t) => {
    ...
    a.value[0] = { foo: 1, bar: 300 };
    t.is(foo, 0);
    t.deepEqual(a.value[0], { foo: 1, bar: 200 });

    a.value[0] = { foo: 2, bar: 300 };
    t.is(foo, 1);
    t.deepEqual(a.value[0], { foo: 2, bar: 300 });
});
