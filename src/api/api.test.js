...
import { sleep } from "../helpers/tools.js";
import { getArrayOfUsedReactiveItems } from "../services/trackers.js";
import { COMPUTED, REACTIVEPROPS_OBJECT } from "../core/Engine.js";
...
test("batch() should throw error if throws in callback", (t) => {
    modeController.batchMode = false;
...
test("reactiveProps", (t) => {
    const a = reactiveProps({ foo: 0 }, { name: "a" });
    t.is(a.foo, 0);
    a.foo = 1;
    t.is(a.foo, 1);
});
...
test("makeObservable 3: Object (reactiveProps, computed)", (t) => {
    let d = {
        value: { foo: 0, bar: 0 },
    ...
    makeObservable(d, { value: "reactiveProps", double: "computed" });
...
test("makeAutoObservable 4: Class (reactiveProps)", (t) => {
    class CustomStore {
        /**
         * customStore.data
    ...
    t.is(computedBar.engine.type, REACTIVEPROPS_OBJECT);
